...
$files_new_at_source = array();
$files_new_at_remote = array();
$counter             = 0;
...
if (isset($_POST['action']) && $_POST['action'] == 'upload') {
    ...
    if (is_array($filePath)) {
        ...
        foreach ($filePath as $path) {
            $file = new CURLFile(realpath($path), getMimeType($path), $path);
        ...
        if ($errorFlag) {
            ...
            $output = json_encode(array('response' => false, 'msg' => $response->msg, 'debug' => curl_error($ch)));
        } else {
            header('Content-Type: application/json; charset=UTF-8');
            $output = json_encode(array('response' => true, 'msg' => FILES_UPLOADED));
        }
        curl_close($ch);
        ...
        $file = new CURLFile(realpath($filePath), getMimeType($filePath), $filePath);
        ...
        if (!$response->error) {
            header('Content-Type: application/json; charset=UTF-8');
            $output = json_encode(array('response' => true, 'msg' => FILE_UPLOADED));
        } else {
            header('HTTP/1.1 500 Internal Server Error');
            //header('HTTP/1.1 401 Unauthorized', true, 401);
            header('Content-Type: application/json; charset=UTF-8');
...
if (isset($_POST['action']) && $_POST['action'] == 'download') {
    ...
    if (is_array($data)) {
        foreach ($data as $file => $encryptedData) {
            ...
            if (makeDirs(dirname($file))) {
                ...
                if (fwrite($fp, $content) === false) {
                    $output = array('response' => false, 'msg' => "Cannot write to file ($file)");
                } else {
                    $output = array('response' => true, 'msg' => FILES_DOWNLOADED);
                }
            } else {
                $output = array('response' => false, 'msg' => "Cannot created path");
            }
        ...
        if (makeDirs(dirname($filename))) {
            ...
            if (fwrite($fp, $content) === false) {
                $output = array('response' => false, 'msg' => "Cannot write to file ($filename)");
            } else {
                $output = array('response' => true, 'msg' => FILE_DOWNLOADED);
            }
        } else {
            $output = array('response' => false, 'msg' => "Cannot created path");
        }
...
if (isset($_POST['action']) && $_POST['action'] == 'delete-source') {
    ...
    if (is_array($filesPath)) {
        ...
        if ($isUnlinked) {
            $output = array('response' => true, 'msg' => FILES_DELETED);
        }
        if ($somethingWrong) {
            $output = array('response' => false, 'msg' => SOMETHING_WRONG);
        }
        if ($isExist == false) {
            $output = array('response' => false, 'msg' => FILE_NOT_PRESENT);
        }
    } else {
        if (file_exists($filesPath)) {
            if (unlink($filesPath)) {
                $output = array('response' => true, 'msg' => FILE_DELETED);
            } else {
                $output = array('response' => false, 'msg' => SOMETHING_WRONG);
            }
        } else {
