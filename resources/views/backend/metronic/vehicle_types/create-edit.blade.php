                    ...
                    <div class="portlet light bordered form-fit">
                        ...
                        <div class="portlet-body form">
	                        ...
	                        @if(strtolower($pageMode) != 'edit')
	                            {!! Form::open(['route' => 'vehicle-type-create', 'class' => 'form-horizontal form-bordered']) !!}
	                        @else
	                            {!! Form::model($vehicleType, ['method' => 'PUT', 'route' => ['vehicle-type-update.vehicle-type', $vehicleType->id], 'class' => 'form-horizontal form-bordered']) !!}
	                        @endif
	                            @include(admin_view('vehicle_types.form'))
	                        {!! Form::close() !!}
                            <!-- END FORM-->
                        </div>
...
@section('foot_resources')
	@if(env('ADMIN_CLIENT_VALIDATIONS', 1))
		...
		@if($pageMode != 'Edit')
			{!! JsValidator::formRequest('App\Http\Requests\Backend\VehicleTypeStoreRequest') !!}
		@else
			{!! JsValidator::formRequest('App\Http\Requests\Backend\VehicleTypeUpdateRequest') !!}
		@endif
	@endif
